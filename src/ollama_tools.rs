...
/// Build a JSON Schema object from parameter tuples.
pub fn make_params(params: &[(&str, &str, bool)]) -> serde_json::Value {
...
pub fn build_tool_definitions(
    ...
    for (name, desc) in mcp_tools.iter().chain(custom_tools.iter()) {
        defs.push(ToolDefinition::new(
            name,
            desc,
            make_params(&[("input", "The input for this tool", true)]),
        ));
    }
    defs
}
...
pub fn parse_tool_calls(response: &str) -> Vec<ToolCall> {
    ...
    static RE: std::sync::LazyLock<regex::Regex> = std::sync::LazyLock::new(|| {
        regex::Regex::new(r"<mcp_tool>(.*?)</mcp_tool>").expect("static regex")
    });
    let mut result = Vec::new();
...
mod tests {
    ...
    #[test]
    fn test_params() {
        let p = make_params(&[("city", "City name", true)]);
